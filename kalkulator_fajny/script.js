...
let alertTimeout; // Zmienna do przechowywania timeoutu

function showErrorAlert(key) {
    ...
    // 2. Ustaw wiadomość
    errorMessage.innerHTML = `Nie wiesz, że klawisza <strong>${key.toUpperCase()}</strong> nie można wciskać?!`;

    // 3. Pokaż alert
    errorAlert.classList.add('visible');

    // 4. Ustaw nowy timeout, aby ukryć alert po 2 sekundach (2000 ms)
...
document.addEventListener('keydown', (event) => {
    const key = event.key;
    const isModifierKey = event.altKey || event.ctrlKey || event.metaKey || event.shiftKey;

    // Ignoruj klawisze modyfikujące (Alt, Ctrl, Shift itp.) i ukryj alert natychmiast
    if (isModifierKey) {
        return;
    }

    // Sprawdź, czy klawisz jest obsługiwany
    ...
    // --- LOGIKA JAJCARSKIEGO ALERTU ---
    // Jeśli żaden z klawiszy nie został obsłużony i klawisz nie jest standardowym modyfikatorem (jak Shift/Ctrl)
    if (!keyHandled) {
        // Ignoruj F-klawisze (F1-F12), strzałki, itp., które nie mają sensownego wyświetlania w alercie
        const ignoredKeys = ['F', 'Arrow', 'Tab', 'Shift', 'Control', 'Alt', 'Meta', 'CapsLock', 'Dead', 'ContextMenu', 'Insert', 'Home', 'End', 'PageUp', 'PageDown', 'NumLock', 'ScrollLock', 'Pause'];
        
        const shouldIgnore = ignoredKeys.some(prefix => key.startsWith(prefix)) || key.length > 1;
